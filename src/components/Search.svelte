...
// Svelte 5: receive named slots (now snippets) from Astro
const { 'search-icon': searchIcon, 'search-switch': searchSwitch, 'arrow-icon': arrowIcon } = $props()
...
">
    {#if searchIcon}{@render searchIcon()}{/if}
...
<button onclick={togglePanel} aria-label="Search Panel" id="search-switch"
    ...
    {#if searchSwitch}{@render searchSwitch()}{/if}
</button>
    ...
    {#each result as item}
       ...
       rounded-xl text-lg px-3 py-2 hover:bg-(--btn-plain-bg-hover) active:bg-(--btn-plain-bg-active)">
            <div class="transition text-90 inline-flex font-bold group-hover:text-(--primary)">
                {item.meta.title}{#if arrowIcon}{@render arrowIcon()}{/if}
            </div>
