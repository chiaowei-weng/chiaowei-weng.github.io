 <center>
     <h1>XXX</h1>
     <div>
         <span>
             <img src="assets/phone-solid.svg" width="18px">
             0900000000
         </span>
         ·
         <span>
             <img src="assets/envelope-solid.svg" width="18px">
             cosmos@gmail.com
         </span>
         ·
         <span>
             <img src="assets/github-brands.svg" width="18px">
             <a href="https://github.com/chiaowei-weng">GitHub</a>
         </span>
         ·
         <span>
             <img src="assets/rss-solid.svg" width="18px">
             <a href="https://chiaowei-weng.github.io/blog-hexo/">My Blog(hexo)</a>
         </span>
                  ·
         <span>
             <img src="assets/rss-solid.svg" width="18px">
             <a href="https://chiaowei-weng.github.io/blog-hugo/">My Blog(hugo)</a>
         </span>
     </div>
 </center>

 ## <img src="assets/info-circle-solid.svg" width="30px"> 個人資訊

 - 男，1991 年出生
 - 求職意向：軟體工程師
 - 工作經驗：0 年
 - 期望薪資：0k

## <img src="assets/graduation-cap-solid.svg" width="30px"> 教育經歷

- 碩士，XXXX大學，計算機科學與技術專業，2016.9~2019.7
- 學士，XXXX大學，軟體工程專業，2012.9~2016.7
-

## <img src="assets/briefcase-solid.svg" width="30px"> 工作經歷

- **XXXX 公司，XXXX 部門，XXXX 工程師，2010.1~2010.9**

   負責 XXX

## <img src="assets/project-diagram-solid.svg" width="30px"> 專案經歷

- **XXXX 專案**

  *使用到的技術*

  使用一兩句話描述專案的主要功能，然後介紹自己在專案中的角色，解決了什麼問題，使用什麼方式解決，比別人的方法相比有什麼優勢（儘量用資料來說明）。

## <img src="assets/tools-solid.svg" width="30px"> 技能清單

- ★★★ MySQL
- ★★★ Redis
- ★★☆ JavaScript
- ★★☆ PHP